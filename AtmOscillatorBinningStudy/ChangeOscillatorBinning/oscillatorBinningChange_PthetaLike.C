...
void oscillatorBinningChange_PthetaLike(){
  ...
  double ptheta_x[35] = {0., 0.1855299, 0.2848429,0.4,0.6,0.7502826,1,1.347294,1.785002,2.329729,3,3.526656,4.124808,4.801501,5.564227,6.363398,7.316889,8.501918,9.205422,10,10.90208,11.93196,13.11498,14.48307,16.07695,17.94919,20.16859,22.8266,26.04689,30.,37.,50.,100.,1000.,100000.};
  for(int i=0; i<sizeof(ptheta_x)/sizeof(ptheta_x[0]);i++){
    new_coarse_x_edges.push_back(ptheta_x[i]);
  } 
  double ptheta_y[16] = {-1., -0.9343027, -0.8376, -0.7583535,-0.6428983,-0.5483458,-0.4464,-0.2980115,-0.1765987,-0.04579432,0.09509569,0.2468142,0.4101562, 0.5859725, 0.775173, 1.};
  for(int i=0; i<sizeof(ptheta_y)/sizeof(ptheta_y[0]);i++){
    new_coarse_y_edges.push_back(ptheta_y[i]);
  } 
  ...
  int fine_per_coarse_x = 20;
  int fine_per_coarse_y = 40;
  std::vector<double> new_fine_x_edges;
  std::vector<double> new_fine_y_edges;
  
  new_fine_x_edges = ReturnFineBinningFromCoarseBinning(fine_per_coarse_x, new_coarse_x_edges); 
  new_fine_y_edges = ReturnFineBinningFromCoarseBinning(fine_per_coarse_y, new_coarse_y_edges);
