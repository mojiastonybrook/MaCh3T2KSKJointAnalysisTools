...
void oscillatorBinningChange_fromFineToCoarse(){
  ...
  std::vector<double> fine_x_edges(fine_x_bins+1);
  std::vector<double> fine_y_edges(fine_y_bins+1);
  ...
  int merging_adj_bins = 1;  // how many adjacent bins to be merged in fine
  int fine_to_coarse = 10;   // how many adjacent bins to form a coarse bin, 10 -- default
  for(int i=0; i< fine_x_edges.size()-1; i++){
    //std::cout << i%merging_adj_bins <<" "<< fine_x_edges[i] <<std::endl;
    if(i%merging_adj_bins == 0){
      new_fine_x_edges.push_back(fine_x_edges[i]); 
    }  
    if(i%fine_to_coarse == 0){
      new_coarse_x_edges.push_back(fine_x_edges[i]); 
    }  
  }
  new_fine_x_edges.push_back(fine_x_edges[fine_x_bins]);  // add the upper limit back
  new_coarse_x_edges.push_back(fine_x_edges[fine_x_bins]);
  int fine_to_coarse_y = 5; // 10 -- default;
                           // 
  for(int i=0; i< fine_y_edges.size()-1; i++){
    if(i%merging_adj_bins == 0){
      new_fine_y_edges.push_back(fine_y_edges[i]);
    }  
    if(i%fine_to_coarse_y == 0){
      new_coarse_y_edges.push_back(fine_y_edges[i]);
    } 
  }
  new_fine_y_edges.push_back(fine_y_edges[fine_y_bins]);  // add the upper limit back
  new_coarse_y_edges.push_back(fine_y_edges[fine_y_bins]);
