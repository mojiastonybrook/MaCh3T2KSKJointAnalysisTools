...
void ReweightPrior_fast(std::string ReducedChain, int prior, std::string OutputFile) {
  if (prior == -1 ){
  ...
  if (fin->Get("osc_posteriors")){
    ...
    reduced = true; // using a chain with osc and nuisance parameter wo ndd
  }
  else {
  ...
  treeold->SetBranchStatus("*", true);
  TTree *tree = treeold->CloneTree(-1, "fast");
  fin->Close();
  delete fin;
  ...
  // Save every 5M steps
  tree->SetAutoSave(5E7);

  Long64_t nEntries = tree->GetEntries();
  std::cout << "Reweighting chain with " << nEntries << " steps" << std::endl;
  for (Long64_t i = 0; i < nEntries; ++i){
    tree->GetEntry(i);
    if (i % 1000000 == 0) std::cout << "Step " << i << "/" << nEntries << " (" << double(i)/double(nEntries)*100. << "%)" << std::endl;
...
// When not specifying the output file
void ReweightPrior_fast(std::string ReducedChain, int prior = -1) {
  // First get in/out name
  std::size_t pos;
  pos = ReducedChain.find(".root");
  std::string sout = ReducedChain.substr(0,pos) + "_reweighted_fast.root";
  ReweightPrior_fast(ReducedChain, prior, sout);
}

int main(int argc, char **argv) {
  if (argc == 4) ReweightPrior_fast(std::string(argv[1]), std::atoi(argv[2]), std::string(argv[3]));
  else if (argc == 3) ReweightPrior_fast(std::string(argv[1]), std::atoi(argv[2]));
  else if (argc == 2) ReweightPrior_fast(std::string(argv[1]));
  else {
    std::cerr << "Need at least one argument, the input file" << std::endl;
    throw;
  }
  return 0;
}
